...
HTML;
    ...
    private function printProductInfo(
        ...
        if ($action->isActionList()) {
            ...
            $printResult = $this->printRequestData($request, $listingProduct, $action->getConfigurator());
        ...
        $currentStatusTitle = \M2E\OnBuy\Model\Product::getStatusTitle($listingProduct->getStatus());
...
HTML;
    ...
    private function printRequestData(
        \M2E\OnBuy\Model\Product\Action\AbstractRequest $request,
    ...
    public function getPrintOrderQuoteDataAction()
    {
        ...
        $buildResultHtml = '';
        if ($isPrint && $orderId) {
            $orderResource = $this->_objectManager->create(\M2E\OnBuy\Model\ResourceModel\Order::class);
            $order = $this->_objectManager->create(\M2E\OnBuy\Model\Order::class);
            ...
            $magentoQuoteBuilder = $this
                ->_objectManager
                ->create(\M2E\OnBuy\Model\Magento\Quote\Builder::class, ['proxyOrder' => $proxy]);

            $magentoQuoteManager = $this
                ->_objectManager
                ->create(\M2E\OnBuy\Model\Magento\Quote\Manager::class);
...
HTML;
    ...
    public function getInspectorDataAction()
    {
        if ($this->getRequest()->getParam('print')) {
            ...
            $listingProduct = $this
                ->_objectManager
                ->create(\M2E\OnBuy\Model\Product\Repository::class)
                ->get($listingProductId);

            $instructionCollection = $this
                ->_objectManager
                ->create(\M2E\OnBuy\Model\ResourceModel\Instruction\CollectionFactory::class)
                ->create();
            ...
            $checkerInput = $this
                ->_objectManager
                ->create(\M2E\OnBuy\Model\Instruction\SynchronizationTemplate\Checker\InputFactory::class)
            ...
            $notListedChecker = $this
                ->_objectManager
                ->create(\M2E\OnBuy\Model\Instruction\SynchronizationTemplate\Checker\CheckerFactory::class)
                ->create(
                    \M2E\OnBuy\Model\Instruction\SynchronizationTemplate\Checker\NotListedChecker::class,
                    $checkerInput,
                );
            ...
            $inactiveChecker = $this
                ...
                ->create(\M2E\OnBuy\Model\Instruction\SynchronizationTemplate\Checker\CheckerFactory::class)
                ->create(
                    \M2E\OnBuy\Model\Instruction\SynchronizationTemplate\Checker\InactiveChecker::class,
                    $checkerInput,
                );
            ...
            $activeChecker = $this
                ...
                ->create(\M2E\OnBuy\Model\Instruction\SynchronizationTemplate\Checker\CheckerFactory::class)
                ->create(
                    \M2E\OnBuy\Model\Instruction\SynchronizationTemplate\Checker\ActiveChecker::class,
                    $checkerInput,
                );
