...
abstract class AbstractMain extends AbstractBase
{
    ...
    protected function preDispatch(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        if ($request->isGet() && !$request->isPost() && !$request->isXmlHttpRequest()) {
            /** @var \M2E\OnBuy\Helper\Module\Exception $exceptionHelper */
            $exceptionHelper = $this->_objectManager->get(\M2E\OnBuy\Helper\Module\Exception::class);
            try {
            ...
            try {
                /** @var \M2E\OnBuy\Model\Servicing\Dispatcher $dispatcher */
                $dispatcher = $this->_objectManager->get(\M2E\OnBuy\Model\Servicing\Dispatcher::class);
     ...
     *
     * @return $this|\M2E\OnBuy\Controller\Adminhtml\AbstractBase|\Magento\Framework\App\ResponseInterface
     */
    ...
    protected function appendMSINotificationPopup()
    {
        ...
        /** @var \M2E\OnBuy\Model\MSI\Notification\Manager $msiNotificationManager */
        $msiNotificationManager = $this->_objectManager->get(\M2E\OnBuy\Model\MSI\Notification\Manager::class);
        if (!$msiNotificationManager->isNeedShow()) {
            return;
        }

        $block = $this->getLayout()->createBlock(\M2E\OnBuy\Block\Adminhtml\MsiNotificationPopup::class);
    ...
    protected function getRedirectToWizard()
    {
        ...
        $wizardHelper = $this->_objectManager->get(\M2E\OnBuy\Helper\Module\Wizard::class);
    ...
    protected function getCustomViewHelper(): \M2E\OnBuy\Helper\View\OnBuy
    {
        return $this->getViewHelper()->getViewHelper();
    }

    protected function getCustomViewControllerHelper(): \M2E\OnBuy\Helper\View\OnBuy\Controller
    {
    ...
    private function addNotificationMessages(): void
    {
        if (
            ...
            $moduleHelper = $this->_objectManager->get(\M2E\OnBuy\Helper\Module::class);
    ...
    private function addStaticContentNotification(): bool
    {
        ...
        if (!$moduleHelper->isStaticContentDeployed()) {
            $this->addExtendedErrorMessage(
                __(
                    '<p>M2E OnBuy Connect interface cannot work properly and there is no way to ' .
    ...
    private function addStaticContentWarningNotification(): void
    {
        ...
        /** @var \M2E\OnBuy\Model\Module $moduleModel */
        $moduleModel = $this->_objectManager->get(\M2E\OnBuy\Model\Module::class);
        $skipMessageForVersion = $this->_objectManager->get(\M2E\OnBuy\Model\Registry\Manager::class)->getValue(
        ...
        $lastUpgrade = $setupResource->findLastUpgrade(\M2E\OnBuy\Helper\Module::IDENTIFIER);
        if ($lastUpgrade === null) {
            return;
        }
        ...
        $this->addExtendedWarningMessage(
            __(
                '<p>Static content data was not deployed during the last M2E OnBuy Connect ' .
    ...
    {
        /** @var \M2E\OnBuy\Model\HealthStatus\CurrentStatus $currentStatus */
        $currentStatus = $this->_objectManager->get(\M2E\OnBuy\Model\HealthStatus\CurrentStatus::class);
        /** @var \M2E\OnBuy\Model\HealthStatus\Notification\Settings $notificationSettings */
        $notificationSettings = $this->_objectManager->get(
            \M2E\OnBuy\Model\HealthStatus\Notification\Settings::class
        );
        ...
        /** @var \M2E\OnBuy\Model\HealthStatus\Notification\MessageBuilder $messageBuilder */
        $messageBuilder = $this->_objectManager->get(
            \M2E\OnBuy\Model\HealthStatus\Notification\MessageBuilder::class
        );
    ...
    protected function addCronErrorMessage(): void
    {
        /** @var \M2E\OnBuy\Helper\Module $moduleHelper */
        $moduleHelper = $this->_objectManager->get(\M2E\OnBuy\Helper\Module::class);
        /** @var \M2E\OnBuy\Helper\Module\Cron $moduleCronHelper */
        $moduleCronHelper = $this->_objectManager->get(\M2E\OnBuy\Helper\Module\Cron::class);

        if (!$moduleCronHelper->isModeEnabled()) {
            $this->getMessageManager()->addWarning(
                __(
                    'Automatic Synchronization is disabled. You can enable it under ' .
                    '<i>Stores > Settings > Configuration > M2E OnBuy Connect > Module & Channels > ' .
        ...
        if (
            ...
            $message = __(
                ...
                ' It does not allow M2E OnBuy Connect to work correctly.' .
    ...
    protected function addLicenseActivationNotifications(): bool
    {
        ...
        if (
            ...
            /** @var \M2E\OnBuy\Helper\View\Configuration $configurationHelper */
            $configurationHelper = $this->_objectManager->get(\M2E\OnBuy\Helper\View\Configuration::class);

            $url = $configurationHelper->getLicenseUrl($params);

            $message = __(
                'M2E OnBuy Connect Module requires activation. Go to the <a href="%1" target ="_blank">License Page</a>.',
                $url
            );
    ...
    {
        /** @var \M2E\OnBuy\Helper\Module\Wizard $wizardHelper */
        $wizardHelper = $this->_objectManager->get(\M2E\OnBuy\Helper\Module\Wizard::class);
        /** @var \M2E\OnBuy\Helper\View\Configuration $configurationHelper */
        $configurationHelper = $this->_objectManager->get(\M2E\OnBuy\Helper\View\Configuration::class);
        ...
        if (!$license->getInfo()->getDomainIdentifier()->isValid()) {
            ...
            $message = __(
                'M2E OnBuy Connect License Key Validation is failed for this Domain. ' .
        ...
        if (!$license->getInfo()->getIpIdentifier()->isValid()) {
            ...
            $message = __(
                'M2E OnBuy Connect License Key Validation is failed for this IP. ' .
                'Go to the <a href="%1" target="_blank">License Page</a>.',
                $url
            );
    ...
    private function isContentLocked(): bool
    {
        return $this->_objectManager->get(\M2E\Core\Helper\Magento::class)->isModeProduction()
            && !$this->_objectManager->get(\M2E\OnBuy\Helper\Module::class)->isStaticContentDeployed();
    }
    ...
    private function isContentLockedByWizard(): bool
    {
        ...
        /** @var \M2E\OnBuy\Helper\Module\Wizard $moduleWizardHelper */
        $moduleWizardHelper = $this->_objectManager->get(\M2E\OnBuy\Helper\Module\Wizard::class);
