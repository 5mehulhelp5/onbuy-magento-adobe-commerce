...
class View extends \M2E\OnBuy\Controller\Adminhtml\Listing\Wizard\StepAbstract
{
    ...
    private \M2E\OnBuy\Model\Magento\Product\RuleFactory $magentoProductRuleFactory;
    private \M2E\OnBuy\Model\Listing\Wizard\Manager $manager;
    private \M2E\OnBuy\Helper\Data\Session $sessionHelper;

    public function __construct(
        \M2E\OnBuy\Helper\Data\GlobalData $globalDataHelper,
        \M2E\OnBuy\Model\Magento\Product\RuleFactory $magentoProductRuleFactory,
    ...
    protected function process(\M2E\OnBuy\Model\Listing $listing)
    {
        ...
        if ($source === \M2E\OnBuy\Block\Adminhtml\Listing\Wizard\ProductSourceSelect::MODE_PRODUCT) {
        ...
        if ($source === \M2E\OnBuy\Block\Adminhtml\Listing\Wizard\ProductSourceSelect::MODE_CATEGORY) {
    ...
    private function showGridByCatalog(\M2E\OnBuy\Model\Listing $listing, string $source)
    {
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            $this->setAjaxContent(
                $this->getLayout()
                     ->createBlock(
                         \M2E\OnBuy\Block\Adminhtml\Listing\Wizard\Product\Add\Grid::class,
                     )
                     ->toHtml(),
            );
        ...
        $this->addContent(
            $this->getLayout()->createBlock(
                \M2E\OnBuy\Block\Adminhtml\Listing\Wizard\Product\Add::class,
                '',
                [
                    'sourceMode' => $source,
                ],
    ...
    private function showGridByCategories(\M2E\OnBuy\Model\Listing $listing, string $source)
    {
        $this->setRuleData('product_add_step_one', $listing);
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            /** @var \M2E\OnBuy\Block\Adminhtml\Listing\Wizard\Product\Add\Category\Grid $grid */
            $grid = $this->getLayout()
                         ->createBlock(
                             \M2E\OnBuy\Block\Adminhtml\Listing\Wizard\Product\Add\Category\Grid::class,
                         );
        ...
        /** @var \M2E\OnBuy\Block\Adminhtml\Listing\Wizard\Product\Add $gridContainer */
        ...
        if (empty($data['current_category_id'])) {
            ...
            if (!$currentNode) {
                throw new \M2E\OnBuy\Model\Exception('No Categories found');
            }
    ...
    private function setRuleData(string $prefix, \M2E\OnBuy\Model\Listing $listing): void
    {
        ...
        $this->globalDataHelper->setValue(
            'rule_prefix',
            $prefix,
        ); // \M2E\OnBuy\Block\Adminhtml\Magento\Product\Grid:361
        ...
        $this->globalDataHelper->setValue(
            'rule_model',
            $ruleModel,
        ); // \M2E\OnBuy\Block\Adminhtml\Magento\Product\Grid:69
    }
}
