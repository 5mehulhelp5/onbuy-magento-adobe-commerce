...
namespace M2E\OnBuy\Helper\Component\OnBuy\Template\Switcher;

class DataLoader
{
    private \Magento\Store\Model\StoreManagerInterface $storeManager;
    private \M2E\OnBuy\Model\Policy\Manager $templateManager;
    private \M2E\OnBuy\Model\Policy\ManagerFactory $templateManagerFactory;
    ...
    public function __construct(
        ...
        \M2E\OnBuy\Model\Policy\ManagerFactory $templateManagerFactory,
    ...
    public function load($source, array $params = [])
    {
        ...
        if ($source instanceof \M2E\OnBuy\Helper\Data\Session) {
            $data = $this->getDataFromSession($source, $params);
        }
        if ($source instanceof \M2E\OnBuy\Model\Listing) {
            $data = $this->getDataFromListing($source, $params);
        }
        if ($source instanceof \M2E\OnBuy\Model\ResourceModel\Product\Collection) {
        ...
        $global->setValue('onbuy_account', $account);
        $global->setValue('onbuy_store', $this->storeManager->getStore($storeId));
        $global->setValue('onbuy_attribute_sets', $attributeSets);
        $global->setValue('onbuy_attributes', $attributes);
        $global->setValue('onbuy_display_use_default_option', $displayUseDefaultOption);
        ...
        foreach ($data['templates'] as $nick => $templateData) {
            ...
            $global->setValue("onbuy_template_{$nick}", $template);
            $global->setValue("onbuy_template_mode_{$nick}", $templateData['mode']);
            $global->setValue("onbuy_template_force_parent_{$nick}", $templateData['force_parent']);
        }
    }

    //########################################

    private function getDataFromSession(\M2E\OnBuy\Helper\Data\Session $source, array $params = [])
    {
        if (!isset($params['session_key'])) {
            throw new \M2E\OnBuy\Model\Exception\Logic('Session key is not defined.');
        }
        ...
        foreach ($this->templateManager->getAllTemplates() as $nick) {
            ...
            if (empty($templateMode)) {
                $templateMode = \M2E\OnBuy\Model\Policy\Manager::MODE_CUSTOM;
            }
    ...
    private function getDataFromListing(\M2E\OnBuy\Model\Listing $source, array $params = [])
    {
    ...
    {
        /** @var \M2E\OnBuy\Model\Product $listingProductFirst */
            ...
            if ($source->getSize() <= 200) {
                foreach ($source->getItems() as $listingProduct) {
                    ...
                    if ($templateId != $currentProductTemplateId || $templateMode != $currentProductTemplateMode) {
                        $templateId = null;
                        $templateMode = \M2E\OnBuy\Model\Policy\Manager::MODE_PARENT;
                        $forceParent = true;
                        break;
                    }
                }
    ...
    private function getDataFromRequest(\Magento\Framework\App\RequestInterface $source, array $params = [])
    {
        ...
        $mode = $source->getParam('mode', \M2E\OnBuy\Model\Policy\Manager::MODE_CUSTOM);
    ...
    {
        if ($source instanceof \M2E\OnBuy\Model\Policy\SellingFormat) {
            return \M2E\OnBuy\Model\Policy\Manager::TEMPLATE_SELLING_FORMAT;
        }

        if ($source instanceof \M2E\OnBuy\Model\Policy\Synchronization) {
            return \M2E\OnBuy\Model\Policy\Manager::TEMPLATE_SYNCHRONIZATION;
        }

        if ($source instanceof \M2E\OnBuy\Model\Policy\Shipping) {
