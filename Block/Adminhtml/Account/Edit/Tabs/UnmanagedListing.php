...
use M2E\OnBuy\Model\Account\Settings\UnmanagedListings as UnmanagedListingsSettings;
...
class UnmanagedListing extends AbstractForm
{
    ...
    {
        $unmanagedListingSettings = new \M2E\OnBuy\Model\Account\Settings\UnmanagedListings();
        ...
        $fieldset->addField(
            ...
            [
                ...
                'class' => 'OnBuy-require-select-attribute',
                'values' => [
                    1 => __('Yes'),
                    0 => __('No'),
                ],
        ...
        $fieldset = $form->addFieldset(
            'magento_block_onbuy_accounts_other_listings_product_mapping',
            [
...
HTML
        ...
        foreach ($attributes as $attribute) {
            $attrs = ['attribute_code' => $attribute['code']];
            if (
                $unmanagedListingSettings->isMappingByOpcEnabled()
                && $unmanagedListingSettings->getMappingAttributeByOpc() === $attribute['code']
            ) {
                $attrs['selected'] = 'selected';
            }
            $preparedAttributes[] = [
                'attrs' => $attrs,
                'value' => UnmanagedListingsSettings::MAPPING_OPC_MODE_CUSTOM_ATTRIBUTE,
                'label' => $attribute['label'],
            ];
        }
        ...
        $fieldset->addField(
            ...
            self::SELECT,
            [
                'name' => 'other_listings_mapping[opc][mode]',
                'label' => __('OPC'),
                'class' => 'attribute-mode-select',
                'style' => 'float:left; margin-right: 15px;',
                'values' => [
                    UnmanagedListingsSettings::MAPPING_OPC_MODE_NONE => __('None'),
                    [
                        'label' => __('Magento Attributes'),
                        'value' => $preparedAttributes,
                        'attrs' => [
                ...
                'value' => !$unmanagedListingSettings->isMappingByOpcEnabled() ? $unmanagedListingSettings->getMappingByOpcMode() : '',
                'create_magento_attribute' => true,
            ]
        )->setAfterElementHtml(
            <<<HTML
<div id="mapping_opc_priority">
    {$this->__('Priority')}: <input style="width: 50px;"
                                    name="other_listings_mapping[opc][priority]"
                                    value="{$unmanagedListingSettings->getPriorityForMappingByOpc()}"
                                    type="text"
                                    class="input-text admin__control-text required-entry _required">
</div>
HTML
        ...
        $fieldset->addField(
            'mapping_opc_attribute',
            'hidden',
            [
                'name' => 'other_listings_mapping[opc][attribute]',
                'value' => $unmanagedListingSettings->getMappingAttributeByOpc(),
            ]
        );

        // ----------------------------------------

        $sites = $this->account->getSites();

        if (!empty($sites)) {
            $fieldset = $form->addFieldset(
                'magento_block_accounts_other_listings_related_store_views',
                [
            ...
            foreach ($sites as $site) {
                $fieldset->addField(
                    'related_store_id_' . $site->getId(),
                    self::STORE_SWITCHER,
                    [
                        'label' => $site->getName(),
                        'value' => $unmanagedListingSettings->getRelatedStoreForSiteId($site->getId()),
                    ]
                );
            }
        }
