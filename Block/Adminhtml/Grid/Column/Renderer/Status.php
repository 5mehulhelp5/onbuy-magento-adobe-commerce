...
class Status extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\Options
{
    ...
    public function render(\Magento\Framework\DataObject $row): string
    {
        ...
        if ($this->getColumn()->getData('showLogIcon')) {
            /** @var \M2E\OnBuy\Block\Adminhtml\Grid\Column\Renderer\ViewLogIcon\Listing $viewLogIcon */
            $viewLogIcon = $this->getLayout()->createBlock(
                \M2E\OnBuy\Block\Adminhtml\Grid\Column\Renderer\ViewLogIcon\Listing::class,
                '',
                [
                    'data' => ['jsHandler' => 'OnBuyListingViewOnBuyGridObj'],
                ]
            );
...
HTML;
    ...
    private function getProgressTag($row): string
    {
        ...
        switch ($productLock->getInitiator()) {
            case \M2E\OnBuy\Model\Product\Action\Async\DefinitionsCollection::ACTION_LIST:
                $html .= '<br/><span style="color: #605fff">[List is in progress...]</span>';
                break;

            case \M2E\OnBuy\Model\Product\Action\Async\DefinitionsCollection::ACTION_RELIST:
                $html .= '<br/><span style="color: #605fff">[Relist is in progress...]</span>';
                break;

            case \M2E\OnBuy\Model\Product\Action\Async\DefinitionsCollection::ACTION_REVISE:
                $html .= '<br/><span style="color: #605fff">[Revise is in progress...]</span>';
                break;

            case \M2E\OnBuy\Model\Product\Action\Async\DefinitionsCollection::ACTION_STOP:
                $html .= '<br/><span style="color: #605fff">[Stop is in progress...]</span>';
                break;

            case \M2E\OnBuy\Model\Product\Action\Async\DefinitionsCollection::ACTION_DELETE:
