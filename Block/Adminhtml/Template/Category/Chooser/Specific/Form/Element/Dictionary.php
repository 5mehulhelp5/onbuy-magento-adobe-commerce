...
namespace M2E\OnBuy\Block\Adminhtml\Template\Category\Chooser\Specific\Form\Element;

use M2E\OnBuy\Block\Adminhtml\Template\Category\Chooser\Specific\Form\Element\Dictionary\Multiselect;
...
class Dictionary extends \Magento\Framework\Data\Form\Element\AbstractElement
{
    ...
    public function getAttributeIdHiddenHtml($index, $specific): string
    {
        $element = $this->_factoryElement->create('hidden', [
            'data' => [
                ...
                'class' => 'onbuy-dictionary-specific-attribute-id collected-attribute',
                'value' => $specific['id'],
            ],
        ]);
    ...
    public function getModeHtml($index): string
    {
        $element = $this->_factoryElement->create('hidden', [
            'data' => [
                ...
                'value' => \M2E\OnBuy\Model\Template\Category::MODE_ITEM_SPECIFICS,
            ],
        ]);
    ...
    {
        $values = [
            \M2E\OnBuy\Model\Template\Category::VALUE_MODE_NONE => [
                'value' => \M2E\OnBuy\Model\Template\Category::VALUE_MODE_NONE,
                'label' => __('None'),
            ],
            ...
            \M2E\OnBuy\Model\Template\Category::VALUE_MODE_CUSTOM_VALUE => [
                'value' => \M2E\OnBuy\Model\Template\Category::VALUE_MODE_CUSTOM_VALUE,
                'label' => __('Custom Value'),
            ],
        ];

        if ($specific['required']) {
            $values[\M2E\OnBuy\Model\Template\Category::VALUE_MODE_NONE] = [
                'label' => '',
                'value' => '',
                'style' => 'display: none',
            ];
        ...
        if ($specific['type'] === \M2E\OnBuy\Model\Template\Category::RENDER_TYPE_TEXT) {
            unset($values[\M2E\OnBuy\Model\Template\Category::VALUE_MODE_ONBUY_RECOMMENDED]);
        }
        ...
        $element = $this->_factoryElement->create('select', [
            'data' => [
                ...
                'style' => 'width: 100%',
                'onchange' => "OnBuyTemplateCategorySpecificsObj.dictionarySpecificModeChange('{$index}', this);",
        ...
        $element->setClass('OnBuy-required-when-visible input-specific-value-mode collected-attribute');
        $element->setForm($this->getForm());
    ...
    public function getValueOnBuyRecommendedHtml($index, $specific): string
    {
        $values = [];
        ...
        if (
            $specific['type'] == \M2E\OnBuy\Model\Template\Category::RENDER_TYPE_SELECT_MULTIPLE ||
            $specific['type'] == \M2E\OnBuy\Model\Template\Category::RENDER_TYPE_SELECT_MULTIPLE_OR_TEXT
        ) {
            /** @var \Magento\Framework\Data\Form\Element\Select $element */
            $element = $this->_factoryElement->create(
                ...
                [
                    'data' => [
                        ...
                        'name' => $this->makeInputName($index, 'value_onbuy_recommended'),
                        'style' => 'width: 100%;' . $display,
                        'value' => empty($specific['template_attribute']['value_onbuy_recommended'])
                            ? []
                            : $specific['template_attribute']['value_onbuy_recommended'],
                        'values' => $values,
        ...
        $element->addClass('OnBuy-required-when-visible');
        $element->setNoSpan(true);
        $element->setForm($this->getForm());
        $element->setId($this->makeElementId($index, 'value_onbuy_recommended'));
    ...
    public function getValueCustomValueHtml($index, $specific): string
    {
        ...
        $element = $this->_factoryElement->create('text', [
            'data' => [
                ...
                'style' => 'width: 100%;',
                'class' => 'OnBuy-required-when-visible item-specific collected-attribute',
                'value' => $customValues,
                'disabled' => $disabled,
            ],
        ]);
    ...
    public function getValueCustomAttributeHtml($index, $specific): string
    {
        ...
        $element = $this->_factoryElement->create('select', [
            'data' => [
                ...
                'style' => 'width: 100%;' . $display,
                'class' => 'OnBuy-custom-attribute-can-be-created collected-attribute',
