...
namespace M2E\OnBuy\Block\Adminhtml\ControlPanel\Tabs\Database;
...
class Grid extends \M2E\OnBuy\Block\Adminhtml\Magento\Grid\AbstractGrid
{
    ...
    private \M2E\OnBuy\Model\ResourceModel\Collection\CustomFactory $customCollectionFactory;
    private \M2E\OnBuy\Helper\Module\Database\Structure $databaseHelper;
    ...
    public function __construct(
        ...
        \M2E\OnBuy\Helper\Module\Database\Structure $databaseHelper,
        array $data = []
    ) {
    ...
    protected function _prepareCollection()
    {
        ...
        $tablesList = array_unique(array_merge($tablesList, \M2E\OnBuy\Helper\Module\Database\Tables::getAllTables()));

        $collection = $this->customCollectionFactory->create();
        foreach ($tablesList as $tableName) {
            if (
                !\M2E\OnBuy\Helper\Module\Database\Tables::isModuleTable($tableName)
            ...
            $tableRow = [
                ...
                'records' => 0,
                'size' => 0,
                'model' => \M2E\OnBuy\Helper\Module\Database\Tables::getTableModel($tableName),
            ];

            if ($tableRow['is_exist']) {
                $tableRow['size'] = $structureHelper->getDataLengthInMB($tableName);
    ...
    protected function callbackFilterTitle($collection, $column)
    {
        ...
        $this->getCollection()->addFilter(
            'table_name',
            $value,
            \M2E\OnBuy\Model\ResourceModel\Collection\Custom::CONDITION_LIKE
        );
    }
    ...
    protected function callbackFilterMatch($collection, $column)
    {
        ...
        $this->getCollection()->addFilter(
            $field,
            $value,
            \M2E\OnBuy\Model\ResourceModel\Collection\Custom::CONDITION_MATCH
        );
    }
}
