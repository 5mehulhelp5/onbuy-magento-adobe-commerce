...
namespace M2E\OnBuy\Model\Account\Settings;
...
class UnmanagedListings
{
    ...
    public function getMappingTypesByPriority(): array
    {
        ...
        if ($this->isMappingByOpcEnabled()) {
            $types[self::MAPPING_TYPE_BY_OPC] = $this->getPriorityForMappingByOpc();
        }
    ...
    public function getMappingByOpcMode(): int
    {
        return $this->mappingByOpc['mode'];
    }

    public function isMappingByOpcEnabled(): bool
    {
        return $this->mappingByOpc['mode'] === self::MAPPING_OPC_MODE_CUSTOM_ATTRIBUTE;
    }

    public function getMappingAttributeByOpc(): ?string
    {
        return $this->isMappingByOpcEnabled()
            ? $this->mappingByOpc['attribute'] : null;
    }

    public function getPriorityForMappingByOpc(): int
    {
        return $this->mappingByOpc['priority'];
    }
    ...
    public function createWithMappingSettings(
        ...
        if (!empty($byItemId)) {
            $new->mappingByOpc = array_merge($new->mappingByOpc, $this->prepareData($byItemId));
        }
    ...
    {
        return $this->mappingByOpc;
    }

    // ----------------------------------------

    public function getRelatedStoreForSiteId(int $siteId): int
    {
        return $this->getRelatedStores()[$siteId] ?? \Magento\Store\Model\Store::DEFAULT_STORE_ID;
    }
