...
abstract class AbstractSyncProcessor
{
    ...
    public function process(): int
    {
        ...
        if ($this->lockManager->isLocked($this->listingProduct)) {
            $this->actionLogger->logListingProductMessage(
                $this->listingProduct,
                \M2E\OnBuy\Model\Response\Message::createError(
            ...
            if ($apiResponse->isResultError()) {
                ...
                if (!empty($successfulMessage)) {
                    $this->addActionLogMessage(
                        \M2E\OnBuy\Model\Response\Message::createSuccess($successfulMessage)
                    );
                }
            }
    ...
    public function setActionLogger(\M2E\OnBuy\Model\Product\Action\Logger $logger): void
    {
        $this->actionLogger = $logger;
    }

    public function setLogBuffer(\M2E\OnBuy\Model\Product\Action\LogBuffer $logBuffer): void
    {
        $this->logBuffer = $logBuffer;
    }

    public function setLockManager(\M2E\OnBuy\Model\Product\LockManager $lockManager): void
    {
        $this->lockManager = $lockManager;
    }

    public function setListingProduct(\M2E\OnBuy\Model\Product $listingProduct): void
    {
