<?php

declare(strict_types=1);

namespace M2E\OnBuy\Model\Product\Action\Manual\Realtime;

class ReviseAction extends AbstractRealtime
{
    use \M2E\OnBuy\Model\Product\Action\Manual\SkipMessageTrait;

    protected function getAction(): int
    {
        return \M2E\OnBuy\Model\Product::ACTION_REVISE;
    }

    protected function calculateAction(\M2E\OnBuy\Model\Product $product, \M2E\OnBuy\Model\Product\ActionCalculator $calculator): \M2E\OnBuy\Model\Product\Action
    {
        $result = $calculator->calculateToReviseOrStop($product);
        if ($result->isActionStop()) {
            return \M2E\OnBuy\Model\Product\Action::createNothing($product);
        }

        return $result;
    }

    protected function logAboutSkipAction(
        \M2E\OnBuy\Model\Product $product,
        \M2E\OnBuy\Model\Listing\LogService $logService
    ): void {
        $logService->addProduct(
            $product,
            \M2E\Core\Helper\Data::INITIATOR_USER,
            \M2E\OnBuy\Model\Listing\Log::ACTION_REVISE_PRODUCT,
            null,
            $this->createSkipReviseMessage(),
            \M2E\OnBuy\Model\Log\AbstractModel::TYPE_INFO,
        );
    }
}
