...
class Builder
{
    ...
    protected \M2E\OnBuy\Model\Order\ProxyObject $proxyOrder;
    ...
    /** @var \M2E\OnBuy\Model\Magento\Quote\ItemFactory */
    private ItemFactory $magentoQuoteItemFactory;

    public function __construct(
        \M2E\OnBuy\Model\Magento\Quote\Store\ConfiguratorFactory $configuratorFactory,
        ...
        \M2E\OnBuy\Model\Magento\Quote\ItemFactory $magentoQuoteItemFactory
    ) {
    ...
    private function initializeQuote()
    {
        ...
        $this->quote->setIsOnBuyQuote(true);
    ...
    private function initializeAddresses()
    {
        ...
        $billingAddress->setLimitCarrier('onbuyshipping');
        $billingAddress->setShippingMethod('onbuyshipping_onbuyshipping');
        ...
        $shippingAddress->setLimitCarrier('onbuyshipping');
        $shippingAddress->setShippingMethod('onbuyshipping_onbuyshipping');
    ...
    /**
     * @param \M2E\OnBuy\Model\Order\Item\ProxyObject $item
     * @param \M2E\OnBuy\Model\Magento\Quote\Item $quoteItemBuilder
    ...
    protected function initializeQuoteItem($item, $quoteItemBuilder, $product, $request)
    {
        ...
        if (is_string($result)) {
            throw new \M2E\OnBuy\Model\Exception($result);
        }
    ...
    {
        $this->quote->setUseOnBuyDiscount(false);
