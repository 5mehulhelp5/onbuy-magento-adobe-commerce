...
class Processing extends \M2E\OnBuy\Model\ActiveRecord\AbstractModel
{
    ...
    /** @var \M2E\OnBuy\Model\Processing\Repository */
    ...
    private function setType(int $type): self
    {
        if (!in_array($type, [self::TYPE_SIMPLE, self::TYPE_PARTIAL], true)) {
            throw new \M2E\OnBuy\Model\Exception\Logic("Processing type '$type' is not valid.");
        }
    ...
    public function addPartialData(array $data, int $partNumber): self
    {
        if (!$this->isTypePartial()) {
            throw new \M2E\OnBuy\Model\Exception\Logic('Unable add partial data for simple processing');
        }
