...
{
    private \M2E\OnBuy\Model\Category\Dictionary $categoryDictionary;
    private \M2E\OnBuy\Model\Category\Attribute\Repository $attributeRepository;

    /** @var \M2E\OnBuy\Model\Category\CategoryAttribute[] */
    ...
    /**
     * @param \M2E\OnBuy\Model\Magento\Product $magentoProduct
     *
     * @return string[]
     */
    public function validateProduct(\M2E\OnBuy\Model\Magento\Product $magentoProduct): array
    {
    ...
    private function findAttributesForValidation(): array
    {
        ...
        foreach ($this->categoryDictionary->getBrandAttribute() as $attribute) {
            if (!$attribute->isRequired()) {
                unset($categoryAttributesWithCustomValueById[$attribute->getId()]);
            }
        }
