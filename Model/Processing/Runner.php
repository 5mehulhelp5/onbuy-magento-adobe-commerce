...
class Runner
{
    ...
    private \M2E\OnBuy\Model\Connector\Client\Single $connector;
    ...
    public function __construct(
        \M2E\OnBuy\Model\Processing\CreateService $processingCreate,
        \M2E\OnBuy\Model\Connector\Client\Single $connector,
        \M2E\OnBuy\Model\Processing\ResultHandlerCollection $resultHandlerCollection,
        \M2E\OnBuy\Model\Processing\LockManagerFactory $lockManagerFactory
    ) {
    ...
    private function validateHandler(SingleInitiatorInterface $initiator): void
    {
        ...
        if ($initiator instanceof \M2E\OnBuy\Model\Processing\PartialInitiatorInterface) {
            if (!is_a($handlerClass, \M2E\OnBuy\Model\Processing\PartialResultHandlerInterface::class, true)) {
                throw new \M2E\OnBuy\Model\Exception\Logic('Result handler is not valid for this processing.');
            }

            return;
        }

        if (!is_a($handlerClass, \M2E\OnBuy\Model\Processing\SingleResultHandlerInterface::class, true)) {
            throw new \M2E\OnBuy\Model\Exception\Logic('Result handler is not valid for this processing.');
        }
    }
