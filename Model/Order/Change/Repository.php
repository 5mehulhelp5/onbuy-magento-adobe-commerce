...
use M2E\OnBuy\Model\ResourceModel\Order\Change as OrderChangeResource;

class Repository
{
    private OrderChangeResource $resource;
    private \M2E\OnBuy\Model\ResourceModel\Order\Change\CollectionFactory $collectionFactory;
    private \M2E\OnBuy\Model\ResourceModel\Order $orderResource;

    public function __construct(
        OrderChangeResource $resource,
        \M2E\OnBuy\Model\ResourceModel\Order\Change\CollectionFactory $collectionFactory,
        \M2E\OnBuy\Model\ResourceModel\Order $orderResource
    ) {
    ...
    public function create(\M2E\OnBuy\Model\Order\Change $change): void
    {
        $this->resource->save($change);
    }

    public function save(\M2E\OnBuy\Model\Order\Change $change): void
    {
        $this->resource->save($change);
    }

    public function delete(\M2E\OnBuy\Model\Order\Change $change): void
    {
        $this->resource->delete($change);
    }

    public function findExist(int $orderId, string $action, string $hash): ?\M2E\OnBuy\Model\Order\Change
    {
        ...
        /** @var \M2E\OnBuy\Model\Order\Change $change */
    ...
    /**
     * @return \M2E\OnBuy\Model\Order\Change[]
     */
    public function findShippingNotStarted(int $orderId): array
    {
        ...
        $collection
            ...
            ->addFieldToFilter(
                OrderChangeResource::COLUMN_ACTION,
                ['eq' => \M2E\OnBuy\Model\Order\Change::ACTION_UPDATE_SHIPPING]
            )
            ->addFieldToFilter(
     ...
     * @return \M2E\OnBuy\Model\Order\Change[]
     */
    public function findShippingReadyForProcess(\M2E\OnBuy\Model\Account $account, int $limit): array
    {
     ...
     * @return \M2E\OnBuy\Model\Order\Change[]
     */
    public function findCanceledReadyForProcess(\M2E\OnBuy\Model\Account $account, int $limit): array
    {
        return $this->findReadyForProcessByAction(\M2E\OnBuy\Model\Order\Change::ACTION_CANCEL, $account, $limit);
    }
    ...
    private function findReadyForProcessByAction(
        ...
        $collection->joinInner(
            ...
            sprintf(
                ...
                \M2E\OnBuy\Model\ResourceModel\Order::COLUMN_ID,
                OrderChangeResource::COLUMN_ORDER_ID,
            ),
            [
                'account_id' => \M2E\OnBuy\Model\ResourceModel\Order::COLUMN_ACCOUNT_ID,
            ],
        );
        $collection->addFieldToFilter(
            sprintf('orders.%s', \M2E\OnBuy\Model\ResourceModel\Order::COLUMN_ACCOUNT_ID),
            ['eq' => $account->getId()],
        );
