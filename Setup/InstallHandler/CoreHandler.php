...
use M2E\OnBuy\Model\Module\Configuration;
use M2E\OnBuy\Model\ResourceModel\Lock\Item as LockItemResource;
use M2E\OnBuy\Model\ResourceModel\Lock\Transactional as LockTransactionalResource;
use M2E\OnBuy\Model\ResourceModel\OperationHistory as OperationHistoryResource;
use Magento\Framework\DB\Ddl\Table;
use M2E\OnBuy\Model\Cron\Config as CronConfig;
use M2E\OnBuy\Model\Cron\Task\System\Servicing\SynchronizeTask as CronTaskServicing;
...
class CoreHandler implements \M2E\Core\Model\Setup\InstallHandlerInterface
{
    ...
    private function installConfigData(\Magento\Framework\Setup\SetupInterface $setup): void
    {
        ...
        $config->insert(
            \M2E\OnBuy\Model\Connector\Client\Config::CONFIG_GROUP_SERVER,
            \M2E\OnBuy\Model\Connector\Client\Config::CONFIG_KEY_APPLICATION_KEY,
            '01a9a292117a636d3471a3b5be05f111993e3d1f'
        );
        ...
        $config->insert(
            \M2E\OnBuy\Model\Settings::CONFIG_GROUP,
            \M2E\OnBuy\Model\Settings::CONFIG_KEY_IDENTIFIER_CODE_MODE,
            \M2E\OnBuy\Model\Settings::VALUE_MODE_NOT_SET
        );
        $config->insert(
            \M2E\OnBuy\Model\Settings::CONFIG_GROUP,
            \M2E\OnBuy\Model\Settings::CONFIG_KEY_IDENTIFIER_CODE_CUSTOM_ATTRIBUTE,
            ''
        );
        ...
        $config->insert(
            \M2E\OnBuy\Model\Product\InspectDirectChanges\Config::GROUP,
            \M2E\OnBuy\Model\Product\InspectDirectChanges\Config::KEY_MAX_ALLOWED_PRODUCT_COUNT,
            '2000'
        );
