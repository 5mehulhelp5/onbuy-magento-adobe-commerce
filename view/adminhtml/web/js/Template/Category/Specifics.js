define([
    ...
    window.OnBuyTemplateCategorySpecifics = Class.create(Common, {
        ...
        initialize: function () {
            jQuery.validator.addMethod('onbuy-custom-specific-attribute-id', function (value, el) {

                        var customTitleInput = el;

                        var result = true;
                        $$('.onbuy-dictionary-specific-attribute-id').each(function (el) {
                            if (el.value == value) {
                                result = false;
                                throw $break;
                            }
                        ...
                        $$('.onbuy-custom-specific-attribute-id').each(function (el) {
        ...
        dictionarySpecificModeChange: function (index, select) {
            ...
            let recommended = $(`${scope}_dictionary_value_onbuy_recommended_${index}`),
            ...
            if (select.value == OnBuy.php.constant('\\M2E\\OnBuy\\Model\\Template\\Category::VALUE_MODE_ONBUY_RECOMMENDED')) {
                recommended.show().enable();
            }
            if (select.value == OnBuy.php.constant('\\M2E\\OnBuy\\Model\\Template\\Category::VALUE_MODE_CUSTOM_VALUE')) {
                customValueTable.show();
                customValueInputs.invoke('enable');
            }
            if (select.value == OnBuy.php.constant('\\M2E\\OnBuy\\Model\\Template\\Category::VALUE_MODE_CUSTOM_ATTRIBUTE')) {
