define([
    ...
    Order = Class.create(Common, {
        ...
        createHelpTitleHtml: function (rowId) {
            ...
            var closeHtml = '<a href="javascript:void(0);" onclick="OrderObj.hideOrderHelp(' + rowId + ');" title="' + OnBuy.translator.translate('Close') + '"><span class="hl_close icon-close"></span></a>';
        ...
        createHelpActionHtml: function (action) {
            ...
            } else if (action.type == OnBuy.php.constant('M2E_OnBuy_Model_Log_AbstractModel::TYPE_INFO')) {
            ...
            var type = '<span style="color: green;">' + OnBuy.translator.translate('Success') + '</span>';
            if (action.type == OnBuy.php.constant('M2E_OnBuy_Model_Log_AbstractModel::TYPE_INFO')) {
                type = '<span style="color: blue;">' + OnBuy.translator.translate('Info') + '</span>';
            } else if (action.type == OnBuy.php.constant('M2E_OnBuy_Model_Log_AbstractModel::TYPE_WARNING')) {
                type = '<span style="color: orange;">' + OnBuy.translator.translate('Warning') + '</span>';
            } else if (action.type == OnBuy.php.constant('M2E_OnBuy_Model_Log_AbstractModel::TYPE_ERROR')) {
                type = '<span style="color: red;">' + OnBuy.translator.translate('Error') + '</span>';
            }
        ...
        createHelpViewAllLogHtml: function (rowId, gridId) {
            let url = OnBuy.url.get('log_order/index', {id: rowId});

            return '<div class="hl_footer"><a target="_blank" href="' + url + '">' + OnBuy.translator.translate('View Full Order Log') + '</a></div>';
        },
