define([
    ...
    window.SynchProgress = Class.create(Common, {
        ...
        runTask: function (taskTitle, taskUrl, taskCheckUrl, callBackWhenEnd) {
            ...
            var self = this;
            self.start(taskTitle, OnBuy.translator.translate('Preparing to start. Please wait ...'));
            ...
            new Ajax.Request(taskCheckUrl, {
                ...
                onSuccess: function (transport) {
                    ...
                    if (data.ajaxExpired && response.ajaxRedirect) {
                        ...
                        alert(OnBuy.translator.translate('Unauthorized! Please login again.'));
        ...
        printFinalMessage: function () {
            ...
            if (self.result == self.resultTypeError) {
                MessageObj.addError(str_replace(
                        '%url%',
                        OnBuy.url.get('logViewUrl'),
                        OnBuy.translator.translate('Shop synchronization was completed with errors. <a target="_blank" href="%url%">View Log</a> for the details.')
                ));
            } else if (self.result == self.resultTypeWarning) {
                MessageObj.addWarning(str_replace(
                        '%url%',
                        OnBuy.url.get('logViewUrl'),
                        OnBuy.translator.translate('Shop synchronization was completed with warnings. <a target="_blank" href="%url%">View Log</a> for the details.')
                ));
            } else {
                MessageObj.addSuccess(OnBuy.translator.translate('Shop synchronization was completed.'));
            }
