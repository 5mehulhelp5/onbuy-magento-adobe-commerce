define([
    ...
    window.Developers = Class.create({
        ...
        saveDirectDatabaseChanges: function () {
            ...
            new Ajax.Request(OnBuy.url.get('developers/save'), {
                method: 'post',
                asynchronous: true,
                parameters: {
                ...
                onSuccess: function (transport) {
                    ...
                    if (result.success) {
                        MessagesObj.addSuccess(OnBuy.translator.translate('Settings saved'));
                        $('save_inspector_mode').hide();
                        self.inspectionModeValue = self.inspectionModeElement.value;
                    } else {
                        MessagesObj.addError(OnBuy.translator.translate('Error'));
                    }
                }
            });
        }
