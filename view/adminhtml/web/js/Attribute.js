define([], function () {
    window.Attribute = Class.create({
        ...
        checkAttributesSelect: function (id, value) {
            if ($(id)) {
                if (typeof OnBuy.formData[id] != 'undefined') {
        ...
        renderAttributesWithEmptyHiddenOption: function (id, insertTo, value, width) {
            var style = width ? ' style="width: ' + width + 'px;"' : '';
            var txt = '<select name="' + id + '" id="' + id + '" class="OnBuy-required-when-visible select admin__control-select"' + style + '>\n';
        ...
        renderAttributesWithEmptyOption: function (id, insertTo, value, notRequiried) {
            var classes = 'OnBuy-custom-attribute-can-be-created select admin__control-select';

            if (!notRequiried) {
                classes += ' OnBuy-required-when-visible';
            }
