define([
    ...
    window.MovingFromListing = Class.create(Action, {
        ...
        moveToListingGrid: function () {
            let self = this;
            let ignoreListings = JSON.parse(OnBuy.customData.ignoreListings);
            $.ajax(
                    {
                        url: OnBuy.url.get('moveToListingGridHtml'),
                        type: 'GET',
                        data: {
                        ...
                        success: function (response) {
                            self.completeProgressBar();
                            self.openPopUp(response, $t('Moving OnBuy Items'));
                        },
                    }
            );
        },
        ...
        submit: function (listingId, onSuccess) {
            ...
            $.ajax(
                    {
                        url: OnBuy.url.get('moveToListing'),
                        type: 'POST',
                        data: {
                            listingId: listingId
                        },
